...
class Parsing:
    ...
    def cycle_through_files(self):
        ...
        while count < len(sorted_dir):
            ...
            with open(os.path.join(self.full_directory_name, file)) as f:
                ...
                if self.directory_name == "Rucio":
                    try:
                        ...
                        start_time_dt = datetime.datetime(
                            int(Start_Time[0:4]),
                            int(Start_Time[5:7]),
                            int(Start_Time[8:10]),
                        ...
                        print(f"File {file} is giving an error.")
                elif (
                    self.directory_name == "EVNT"
                    or self.directory_name == "TRUTH3"
                    or self.directory_name == "TRUTH3_centos7"
                ):
                    ...
                    new_name = f"{Start_Time[0:4]}.{Start_Time[5:7]}.{Start_Time[8:10]}.T{Start_Time[11:13]}.{Start_Time[14:16]}.{Start_Time[17:19]}.log"
                    os.rename(
                        os.path.join(self.full_directory_name, file),
                        os.path.join(self.full_directory_name, new_name),
                    )
                    my_list.append((end_time_dt - start_time_dt).total_seconds())
        ...
        first_measurement = datetime.datetime(
            int(file_1[0:4]),
            int(file_1[5:7]),
            int(file_1[8:10]),
            int(file_1[12:13]),
        ...
        final_measurement = datetime.datetime(
            int(file_n[0:4]),
            int(file_n[5:7]),
            int(file_n[8:10]),
            int(file_n[12:13]),
...
class plotting(Parsing):
    ...
    def make_the_plot(self):
        horizontal_axis = pd.date_range(
            start=self.first_measurement,
            end=self.final_measurement,
            periods=len(self.my_list),
        )
        fig, ax = plt.subplots()
        ...
        if self.directory_name == "TRUTH3_centos7_interactive":
            ...
            name_on_plot = "TRUTH3"
        plt.title(f"{name_on_plot}", size=15)
